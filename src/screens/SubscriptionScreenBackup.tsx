...
const SubscriptionScreen = ({ navigation }: any) => {
  ...
  const verifyGoogleSubscription = async (purchase: any) => {
    try {
      const token =
        purchase?.purchaseToken || purchase?.transactionReceipt;

      const packageName = 'safe_hunt_subscription_pro';
      const productId = purchase?.productId || purchase?.sku;

      const response = await fetch(
        `https://${API_BASE_URL}/in-app-purchase/verify-google-subscription`,
        {
          method: 'POST',
          body: JSON.stringify({
            token,
